...
class Solution:
    def isValidBST(self, root: Optional[TreeNode]) -> bool:
        ...
        #dfs->explores the left tree to depth before going to right tree: because of "and"
        #t=o(n) :checks all nodes
        #s=o(n)
        #LCA in a BST uses the BST ordering property to follow only one root-to-leaf path, so its complexity is O(h). Validating a BST may require checking every node and its allowed range because a violation can occur anywhere in the tree, so it is O(n).
